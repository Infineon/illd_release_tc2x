...
#define IFXVADC_SYNCHRONIZATION_GROUPS  11
#define IFXVADC_MAX_SLAVE_GROUPS        3
        ...
        for (index = 0; index < IFXVADC_SYNCHRONIZATION_GROUPS; index++)
        {
            ...
            if (((config->slaves >> index) & 0x1) == 1u)
            {
                /* Check if the number of configured slave groups exceeds the allowed maximum
                   OR if the current group is from Group 8-11 (i.e., GroupId_8 to GroupId_11),
                   in which case only 2 slaves are allowed (because one group acts as a master, and only 3 total groups can be involved) */
                if (numSlaves > IFXVADC_MAX_SLAVE_GROUPS || (groupIndex > IfxVadc_GroupId_7 && numSlaves > (IFXVADC_MAX_SLAVE_GROUPS-1)))
                {
